<template>
  <div v-if="avatarImage || selectedImage" class="w-80 h-60 overflow-hidden rounded-md mt-1">
      <img v-if="selectedImage" :src="selectedImage" alt="Selected Cover Image" class="w-full h-full object-cover rounded-md" />
      <img v-else-if="avatarImage !== ''" :src="avatarImage" alt="Cover Image" class="w-full h-full object-cover rounded-md" />
  </div>
...
const { avatarImage } = defineProps(['avatarImage']);
